...
- `/mentor_register` — регистрация ментора; по умолчанию сразу включает карантинный статус `probation`.
...
- Режим `low_staff` по умолчанию: авто-допуск менторов в карантин, матчинг с ограничением, авто-повышение и авто-санкции по подтверждённым жалобам.
- SQLite по умолчанию, SQLAlchemy async для будущего PostgreSQL.
- Docker/Docker Compose для VPS-деплоя.

## Режим low_staff

`low_staff` — основной режим для серверов, где мало администраторов и модераторов.

- Новый ментор после `/mentor_register` получает статус `probation` вместо `pending` и может сразу участвовать в подборе.
- Карантинный ментор ограничен `PROBATION_MAX_NEWBIES=1` активным новичком независимо от заявленного лимита.
- После `PROBATION_SESSIONS=3` завершённых сессий без открытых жалоб бот автоматически переводит ментора в `approved`.
...
- Старый строгий сценарий сохранён: установите `STAFFING_MODE=strict`, и новые менторы снова будут ждать `/admin_approve`.
